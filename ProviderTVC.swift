...
var provider = [Providers]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = provider[indexPath.row].provider
      //  cell.textLabel?.text = " - " + "\(Products.provider)"
        cell.textLabel?.textColor = .lightGray
       cell.detailTextLabel?.text = "\(provider[indexPath.row].parentFolder?.count ?? 0)"
    ...
    */
    func loadProviders(predicate: NSPredicate? = nil) {
             ...
             request.sortDescriptors = [NSSortDescriptor(key: "provider", ascending: true)]
             do {
